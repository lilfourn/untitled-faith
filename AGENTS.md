...
- Use `luke.fournier@gridbloom.app` for Cloudflare sign-in. Production stays in the existing Vendors account (`c0f96de71bdc54889c1ad27ccc90dfc0`); the separate Luke account is not the deployment target. Verify against `backend/wrangler.jsonc` and `backend/DEPLOYMENT.md`.
