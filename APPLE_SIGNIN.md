...
The AI-sharing popup was removed at the user’s request. The Settings switch was also removed; sending a question requests an AI answer, regardless of the old device preference. The backend loads the first name from the authenticated account and includes it as profile data in both streamed and regular answer prompts. An opaque account ID remains the provider’s user identifier; Apple identifiers and credentials are not passed to OpenRouter. No name is inferred from conversation content. Migration `0004_account_first_name.sql` is required before deploying this change.
