    ...
    var sections: [LegalSection] {
        ...
        case .privacy:
            return [
                ...
                LegalSection(title: "Questions and AI answers", body: "When you tap Send, your question, the entire current conversation, and your account’s first name when available are sent through our service hosted by Cloudflare to OpenRouter and Google, with OpenAI as a fallback provider when a model is rate-limited, to generate an AI answer. Anything you include, including personal information, is part of that request. When the assistant searches, OpenRouter sends search queries to Exa; those queries may include details from your conversation. An app-specific identifier is also sent for service operation. We do not send Apple credentials to OpenRouter or the AI providers. The current preview sends requests directly without a separate confirmation popup."),
                LegalSection(title: "Third-party data handling", body: "The service requests Google or OpenAI provider routes through OpenRouter with data collection disabled. This routing setting does not establish a blanket promise that no data is retained. OpenRouter, Google, OpenAI, Cloudflare, and Exa process data under their own policies. The model routing setting does not determine Exa’s retention practices. Account settings and provider terms must also be verified before public launch. Review the linked policies for details."),
                LegalSection(title: "Your choices", body: "You can review this policy before signing in. Sending a question requests an AI answer and shares the current conversation with the answer service. Information already sent cannot be recalled. Use Conversations to delete individual local chats. Use Delete account in Settings to remove this device's saved conversations and sign-in session and revoke Apple authorization. Local conversations on other devices must be deleted on those devices. Previously issued access credentials expire within 15 minutes."),
